...
async function startServer() {
  ...
  async function browserRpc(
    ...
    //    Online Portal
    //
    // The online portal is VIEW ONLY.
    // No business record may be created, edited, deleted,
    // cancelled, voided, returned, paid, or otherwise changed
    // through the browser portal.
    //
    // Authentication/session maintenance remains allowed.
    // ============================================================

    const ONLINE_PORTAL_WRITE_CHANNELS = new Set([
      "users:create",
      "users:update",
      "users:setActive",
      "users:resetPassword",
    ...
    if (ONLINE_PORTAL_WRITE_CHANNELS.has(channel)) {
      return res.status(403).json({
        ok: false,
        code: "ONLINE_PORTAL_READ_ONLY",
        message:
          "Online portal is read-only. Create or modify business data in the Windows application.",
      });
    }
    ...
    /*
     * ============================================================
     * ONLINE PORTAL IS READ-ONLY
     * ============================================================
     *
     * Windows is the production data-entry system.
     *
     * Windows local SQLite -> Neon -> Online Portal
     *
     * The browser portal may READ Neon data, but it must NEVER
     * create, update, archive, void, cancel, pay, or otherwise
     * modify business records.
     *
     * Authentication/session operations remain available.
     */

    const READ_ONLY_PORTAL_BLOCKED = new Set([
      "users:create",
      "users:update",
      "users:setActive",
      "users:resetPassword",
    ...
    if (READ_ONLY_PORTAL_BLOCKED.has(channel)) {
      return res.status(403).json({
        ok: false,
        code: "READ_ONLY_PORTAL",
        message:
          "The online portal is read-only. Business data must be created or changed in the Windows application.",
      });
    }
    ...
    // BANK
    if (channel === "bank:create")
      return core.createBankTransaction(session, payload);

    if (channel === "bank:list")
      return core.listBankTransactions(
        session,
        payload?.from ? new Date(payload.from) : undefined,
        payload?.to ? new Date(payload.to) : undefined
      );

    if (channel === "bank:balance")
      return core.getBankBalance(session);

    if (channel === "bank:transfer")
      return core.transferFunds(session, payload);
