...
export async function recordCustomerPayment(
  ...
  /*
   * Customer payments belonging to a Sale are synchronized by syncSale(),
   * which already loads and upserts all customerPayments.
   *
   * Queue the parent Sale instead of CUSTOMER_PAYMENT because the sync
   * queue processor intentionally handles SALE as the synchronization
   * boundary for the complete sale/payment state.
   */
  if (payment.saleId) {
    await enqueueSync(
      "SALE",
