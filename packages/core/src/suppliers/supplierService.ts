...
export function computeSettlement(totalPurchases: number, totalPaid: number, totalCredit: number) {
...
export async function getSupplierOutstanding(supplierId: string) {
  return (await getSupplierSettlement(supplierId)).outstandingPayable;
}

export async function getSupplierSettlement(supplierId: string) {
  const [purchases, credits, payments] = await Promise.all([
    prisma.purchase.aggregate({ where: { supplierId, status: "COMPLETED" }, _sum: { totalAmount: true } }),
  ...
  return computeSettlement(
    ...
    Number(credits._sum.returnValue ?? 0)
  );
}
...
export async function createSupplier(session: AuthSession, input: SupplierFields) {
...
export async function updateSupplier(session: AuthSession, id: string, input: Partial<SupplierFields>) {
  assertPermission(session, PERMISSIONS.SUPPLIERS_MANAGE);
